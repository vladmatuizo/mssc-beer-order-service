...
public class AllocateOrderAction implements Action<BeerOrderStatusEnum, BeerOrderEventEnum> {
    ...
    public void execute(StateContext<BeerOrderStatusEnum, BeerOrderEventEnum> stateContext) {
        ...
        jmsTemplate.convertAndSend(ALLOCATE_ORDER_QUEUE_NAME,
                new AllocateOrderRequest(beerOrderMapper.beerOrderToDto(beerOrder))
        );

        log.debug("Sent allocation request to queue {} for order with id {}", ALLOCATE_ORDER_QUEUE_NAME, beerOrderId);
    }
}
